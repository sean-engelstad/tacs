...
caps2tacs.PinConstraint("np", dof_constraint=23).register_to(tacs_model)
caps2tacs.PinConstraint("pn", dof_constraint=13).register_to(tacs_model)
caps2tacs.PinConstraint("pp", dof_constraint=3).register_to(tacs_model)

caps2tacs.GridForce("Xn", direction=[0, -1.0, 0.0], magnitude=100.0).register_to(
    tacs_model
)
caps2tacs.GridForce("Xp", direction=[0, +1.0, 0.0], magnitude=100.0).register_to(
    tacs_model
)
caps2tacs.GridForce("Yn", direction=[-1.0, 0.0, 0.0], magnitude=100.0).register_to(
    tacs_model
)
caps2tacs.GridForce("Yp", direction=[1.0, 0.0, 0.0], magnitude=100.0).register_to(
    tacs_model
)
